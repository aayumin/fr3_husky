...
`fr3_husky`는 Franka Research 3(FR3) 로봇 팔과 Husky 모바일 로봇을 제어하기 위한 ROS 2 패키지입니다.
...
```bash
ros2 launch fr3_husky_controller fr3_action_controller.launch.py use_mujoco:=false load_mobile:=false robot_side:=dual
```

### FR3와 Husky 통합 제어

```bash
ros2 launch fr3_husky_controller fr3_husky_action_controller.launch.py use_mujoco:=false load_mobile:=true robot_side:=dual
```
...
2. 아래 명령으로 apple vision pro 노드를 실행합니다.
...
4. 음성 또는 텍스트 명령을 입력하면 AI가 명령을 해석하여 적절한 ROS2 Action Goal을 로봇에 전달합니다.
