...
from fr3_husky_msgs.action import TaskSpaceMove
...
class TaskSpaceMoveClient(Node):
    ...
    def send_goal_and_wait(self):
        ...
        self.get_logger().info(f"Sending TaskSpaceMove goal for arm={arm}")
        ...
        if not goal_handle.accepted:
            self.get_logger().warn("TaskSpaceMove goal rejected")
            return

        self._goal_handle = goal_handle
        self.get_logger().info("TaskSpaceMove goal accepted")
