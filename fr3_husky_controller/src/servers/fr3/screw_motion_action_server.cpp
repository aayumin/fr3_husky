#include <fr3_husky_controller/servers/fr3/screw_motion_action_server.hpp>
...
std::vector<std::string> ScrewMotionBase::eeNamesFromArm(const std::string& arm) const
{
    if (arm != "left" && arm != "right" && arm != "both")
        throw std::invalid_argument("arm must be one of: left, right, both");

    std::vector<std::string> selected;
    for (const auto& ee_name : fr3_model_updater_.ee_names_)
    {
        const bool is_left = ee_name.rfind("left_", 0) == 0;
        const bool is_right = ee_name.rfind("right_", 0) == 0;
        if ((arm == "left" && is_left) ||
            (arm == "right" && is_right) ||
            (arm == "both" && (is_left || is_right)))
        {
            selected.push_back(ee_name);
        }
    }

    if (selected.empty())
        throw std::invalid_argument("requested arm is not configured in the FR3 controller");
    if (arm == "both" && selected.size() != 2)
        throw std::invalid_argument("arm=both requires both left and right arms in the FR3 controller");

    return selected;
}

REGISTER_FR3_ACTION_SERVER(ScrewMotion, "fr3_screw")
REGISTER_FR3_ACTION_SERVER(ScrewMotionZ, "fr3_screw_z")
