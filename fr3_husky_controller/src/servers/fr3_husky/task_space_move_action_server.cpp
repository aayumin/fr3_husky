// task_space_move_action_server.cpp

#include <fr3_husky_controller/servers/fr3_husky/task_space_move_action_server.hpp>
  ...
  fr3_husky_model_updater_(getFR3HuskyModelUpdater(model_updater, name))
{
    ...
    RCLCPP_INFO(node_->get_logger(), "[%s] TaskSpaceMove created", name_.c_str());
}

bool TaskSpaceMove::acceptGoal(const ActionT::Goal& goal)
{
...
void TaskSpaceMove::onGoalAccepted(const ActionT::Goal& goal)
{
...
Eigen::Affine3d TaskSpaceMove::poseMsgToAffine(const geometry_msgs::msg::Pose& msg)
{
...
double TaskSpaceMove::orientationError(const Eigen::Matrix3d& R_des, const Eigen::Matrix3d& R_cur)
{
...
Eigen::Vector6d TaskSpaceMove::computeTargetVelocity(
...
TaskSpaceMove::ComputeResult TaskSpaceMove::compute(
    ...
    const rclcpp::Duration& /*period*/)
{
    ...
    // fb->progress = s;
    // fb->status_message = "Executing task-space move";
    // fb->position_errors = pos_errors;
    // fb->orientation_errors = ori_errors;
    // publishFeedback(fb);
...
TaskSpaceMove::ResultPtr TaskSpaceMove::makeResult(StopReason reason)
{
...
REGISTER_FR3_HUSKY_ACTION_SERVER(TaskSpaceMove, "fr3_husky_task_space_move")
