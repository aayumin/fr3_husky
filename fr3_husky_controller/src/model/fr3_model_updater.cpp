...
bool FR3ModelUpdater::GripperHoming(const std::string robot_name)
{
    if (!has_hand_)
    {
...
bool FR3ModelUpdater::GripperGrasp(const std::string robot_name, const double width, const double speed, const double force, const std::pair<double, double> epsilon)
{
    // 1. Check whether a hand is present in the URDF
    if (!has_hand_)
    {
